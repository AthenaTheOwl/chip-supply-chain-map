...
| R-CDCP-004 | `ops/RELEASE_LEDGER.md` with the four-commit backfill | manual review during commit; future automation may parse the ledger | `owner_role: control.coordinator` |
...
| R-CDCP-007 | `.agents/skills/` folder reserved; first SKILL.md lands later | front-matter parses against `skill.schema.json`; future `validate_skills.py` lands when the first skill graduates | `owner_role: learning.dream-orchestrator` |
