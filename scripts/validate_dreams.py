"""Validate dream records (dreams/<YYYY-WNN>/) in chip-supply-chain-map.
...
This repo has no dreams yet; the validator exits 0 with a "0 dream(s)
found" report until the first week directory lands, mirroring the
no-records handling in sibling validators.
...
# Env-var override mirrors validate_decisions.py so the offline-cache code
# path stays testable end-to-end.
REMOTE_URL = os.environ.get("CHIP_SUPPLY_MAP_SCHEMA_URL_BASE", DEFAULT_REMOTE_URL)
...
def load_remote_schema() -> dict[str, Any] | None:
    try:
        req = urllib.request.Request(
            REMOTE_URL,
            headers={"User-Agent": "chip-supply-chain-map/validate_dreams"},
        )
